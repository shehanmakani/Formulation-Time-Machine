...
if use_synergy:
    machine.add_ingredient(Ingredient("Ferulic Acid", "Stabilizer", 0.5, 0.1, 0.0, 24.0))
...
with col2:
    ...
    st.caption(f"At {temp}°C, degradation is {accel_factor} times faster than room temperature.")

st.markdown("---")
st.subheader("Academic Context & Methodology")
st.latex(r"C(t) = C_0 e^{-kt} \cdot 2^{\frac{T - 25}{10}}")
st.info("""
**Kinetic Modeling:** This simulation utilizes pseudo-first-order kinetics combined with the Arrhenius equation. 
By increasing temperature, we lower the 'effective time' required for the activation energy barrier of L-Ascorbic Acid oxidation to be crossed.
""")
